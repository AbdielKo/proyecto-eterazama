...
import { obtenerNotificaciones, marcarNotificacionLeida, marcarTodasLeidas } from "./chofer.api";
import type { NotificacionChofer } from "./chofer.types";
...
export default function NotificacionesChofer() {
  const [notificaciones, setNotificaciones] = useState<NotificacionChofer[]>([]);

  useEffect(() => {
    obtenerNotificaciones().then(setNotificaciones).catch(console.error);
  }, []);
