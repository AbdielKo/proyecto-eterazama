...
import {
  ...
  marcarNotificacionSecretariaLeida,
  marcarTodasNotificacionesSecretariaLeidas,
} from "../../api/secretaria.api";
import type { NotificacionSecretaria } from "../../api/secretaria.api";
import { useNotificacionesSecretariaSocket } from "../../hooks/useNotificacionesSecretariaSocket";
import { Bell, CheckCheck, Car, Route, Clock, User, MessageSquare } from "lucide-react";

const iconMap: Record<string, typeof Bell> = {
  salida_en_ruta: Route,
  salida_por_salir: Clock,
  fila: Car,
  estado_viaje: Route,
  pasajero: User,
...
export default function NotificacionesSecretaria() {
  ...
  function cargar() {
    obtenerNotificacionesSecretaria().then(setNotificaciones).catch(console.error);
  }

  useNotificacionesSecretariaSocket(cargar);
  ...
  async function handleMarcarLeida(id: string) {
    try {
      await marcarNotificacionSecretariaLeida(id);
  ...
  async function handleMarcarTodas() {
    try {
      await marcarTodasNotificacionesSecretariaLeidas();
      ...
      {notificaciones.length === 0 ? (
        ...
        <div className="space-y-2">
          {notificaciones.map((n) => {
            const Icon = iconMap[n.tipo] || iconMap.default;
            return (
              <div
                key={n.id}
