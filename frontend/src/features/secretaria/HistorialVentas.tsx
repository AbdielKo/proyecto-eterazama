...
// Formato de moneda usado SOLO dentro del modal: Bs 20,00 (es-BO).
...
function ModalDetalleVenta({
  ...
  // Código real del boleto: ECT-XXXXXXXX (codigoBarras || codigo). No se
  // inventa; si no existe en el registro, simplemente no se muestra.
  const codigoBoleto = ventaDetalle.codigoBarras || ventaDetalle.codigo || null;

  // Relación real pasajero <=> asiento (jsonb pasajeros). NO se asigna por
  // posición del arreglo: cada entrada trae su propio campo asiento.
  ...
  // Contacto del recibo: solo si es un dato distinto al comprador (evita duplicar).
  ...
  // La cuenta autenticada que hizo la compra (solo si existe vínculo real).
  ...
  return (
    ...
    >
      ...
      >
        {/* Encabezado: título + código de boleto (lo más visible) */}
        ...
        <div className="p-5 space-y-5">
          ...
          {!cargando && !error && detalle && (
            <>
              {/* 1. Comprador (quien realizó la compra) */}
              ...
              {/* 7. Información técnica (colapsable, secundaria) */}
